...
class TimeFrequencyRecombinationReloaded(generic):
	...
	def transform(self, X):
		...
		for label in np.unique(y_):
			...
			# TF [trials, freq, electrodes, time]
			TF = TF.transpose((0, 2, 1, 3))

			for _ in range(n_new_trials):
				# get the trials to be combined for each frequency
				trials_draw = np.random.randint(0, X_label.shape[0], TF.shape[1])
				new_trial = []
				for freq in range(TF.shape[1]):
					new_trial.append(TF[trials_draw[freq], freq, :, :])

				# new_trial [freq, electrodes, time]
				new_trial = np.array(new_trial)

				# new_trial [electrodes, freq, time]
				new_trial = new_trial.transpose((1, 0, 2))
...
if __name__  == '__main__':
	data = CBCIC().load()	
	augmenter = TimeFrequencyRecombinationReloaded(inplace=True)
	print('Before:')
